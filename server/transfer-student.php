...
>
  <head>
    ...
    <title>Transferred Students | OXFORD COLLEGE V4 | Code by Musaffar Industries</title>
...
?>
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Student /</span> Transferred Students</h4>
              ...
              <div class="row">
                    ...
                    <div class="card-body">
                      <div class="row">
                        <div class="mb-3 col-md-3">
                          ...
                          <?PHP
                            $query2 = "SELECT DISTINCT sttransfrom FROM transfers ORDER BY no ";
                            ...
                            if(mysqli_num_rows($query_run2) > 0)
                            {
                                ...
                                {
                                    ?>
                              <option value="<?=$crs['sttransfrom']?>"><?=$crs['sttransfrom']?></option>
                              <?PHP
                                }
                            }
                            else{
                ...
                <div class="card">
                <h5 class="card-header fw-bold"> Now You Are Seeing Transferred Students
                <?PHP
                  if(isset($_POST['batfcname1'])){
                    echo '[ From '.$_POST['batfcname1'].' ]';
                  }
                  else{
                    echo '[ All ]';
                  }
                  ...
                  <table class="table">
                    ...
                    <?php 
                        ...
                        if(isset($_POST['batfcname1'])){
                          ...
                          $query = "SELECT * FROM transfers WHERE sttransfrom='$batfcname1' ORDER BY no DESC ";
                        }
                        else{
                          $query = "SELECT * FROM transfers ORDER BY no DESC ";
                        }
                        ...
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            foreach($query_run as $sdt)
                            {
                                ?>
                      <tr>
                        <td><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?=$sdt['stregnonew']?></strong></td>
                        <td><?=$sdt['stregno']?></td>
                        <td><?=$sdt['stnameini']?></td>
                        <td><?=$sdt['sttransfrom']?></td>
                        <td><?=$sdt['sttransto']?></td>
                        <td><?=$sdt['sttransdate']?></td>
                        <td><?=$sdt['sttransreason']?></td>
                      </tr>
                      <?php
                            }
                        }
                        else{
                        echo '<tr><td colspan="6" rowspan="2" class="text-center mt-2 mb-2"><center> No Transfers Found </center></td></tr>';
                        }
                        ?>

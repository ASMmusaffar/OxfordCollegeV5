...
>
  <head>
    ...
    <title>DropOuts Overview | OXFORD COLLEGE V4 | Code by Musaffar Industries</title>
...
?>
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Documents /</span> DropOuts</h4>
              <?php include('message.php'); ?>
              <div class="row">
                <div class="col-md-12">
                  <form method="POST" action="" enctype="multipart/form-data">
                  <div class="card mb-4">
                    <h5 class="card-header fw-bold">Filter</h5>
                    <!-- Account -->
                    <div class="card-body">
                      <div class="row">
                        <div class="mb-3 col-md-3">
                          <label class="form-label">Batch</label>
                          <select id="crscname1" name="batfcname1" class="select2 form-select" required>
                            <option value="">Select Batch</option>
                          <?PHP
                            $query2 = "SELECT DISTINCT batfcname FROM students WHERE stdatastatus='1' AND ststatus='dropout' AND sttransto='0' AND batfcname!='' ORDER BY no ";
                            ...
                            if(mysqli_num_rows($query_run2) > 0)
                            {
                                ...
                                {
                                    ?>
                              <option value="<?=$crs['batfcname']?>"><?=$crs['batfcname']?></option>
                              <?PHP
                                }
                            }
                            else{
                        ...
                        <div class="float-end">
                          <button type="submit" name="bat_submit" class="btn btn-primary">Search</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div class="card">
                <h5 class="card-header fw-bold"> Now You Are Seeing
                <?PHP
                  if(isset($_POST['batfcname1'])){
                    echo $_POST['batfcname1'];
                  }
                  else{
                    echo 'All';
                  }
                ...
                ?>  
                DroppedOut Students
                <a href="edit-student.php" class="btn btn-outline-danger float-end">Clear Filter</a>
                </h5>
                <div class="table-responsive text-nowrap">
                  <table class="table">
                    ...
                    <?php 
                        ...
                        if(isset($_POST['batfcname1'])){
                          $batfcname1=$_POST['batfcname1'];
                          $query = "SELECT * FROM students WHERE stdatastatus='1' AND batfcname='$batfcname1' AND ststatus='dropout' ORDER BY no DESC ";
                        }
                        else{
                          $query = "SELECT * FROM students WHERE stdatastatus='1' AND ststatus='dropout' ORDER BY no DESC ";
                        }
                        $query_run = mysqli_query($con, $query);
                        if(mysqli_num_rows($query_run) > 0)
                        {
                            foreach($query_run as $sdt)
                            {
                                ?>
                      <tr>
                        <td><i class="fab fa-angular fa-lg text-danger me-3"></i> <strong><?=$sdt['stregno']?></strong></td>
                        <td><?=$sdt['batfcname']?></td>
                        <td><?=$sdt['stnameini']?></td>
                        <td><?=$sdt['stdrpreason']?></td>
                        <!-- <td>
                          <a 
                          type="submit"
                          href="edit-student-target.php?steditid=<?=$sdt['no']?>"
                      ...
                      <?php
                        ...
                        echo '<tr><td colspan="7" rowspan="2" class="text-center mt-2 mb-2"> No DropOuts Found </td></tr>';
                        }
                        ?>
